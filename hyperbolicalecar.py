...
class CacheMetaData(object):
    def __init__(self):
        ...
        super(CacheMetaData, self).__setattr__("first_ref", -1)
        super(CacheMetaData, self).__setattr__("last_ref", -1000)
...
class hyperbolicALeCar(page_replacement_algorithm):
    def __init__(self, cache_size, learning_rate=0, initial_weight=0.5, discount_rate=1, visualize=0):
        ...
        print("BOEB")
        ss = max( int ( math.log(self.N) ) , 2)
        
        self.SampleSize = min(self.N, ss)
    ...
    def selectEvictPage(self, policy):
        ...
        ss = self.SampleSize
        if self.time % self.N <= 5:
          Sample = range(0, self.N)
        else:  
          Sample = random.sample(self.IndexArray, ss)
        ...
        for pageIndex in Sample:
            ...
            if f is None:
                ...
                if (h.time) < (pageBlock.time):
                    f = pageBlock
                    
#                if (pageBlock.freq == 1):
#                    if(h.freq != 1):
#                      h = pageBlock
#                    elif((h.time) > (pageBlock.time)): 
#                      h = pageBlock 
                if (h.time) > (pageBlock.time):#float(h.freq) / float(self.time - h.last_ref) > float(pageBlock.freq) / float(self.time - pageBlock.last_ref):
                    h = pageBlock
    ...
    def evictPageAndAddToCache(self, index, old_page, new_page):
        new_page.last_ref = new_page.time
